...
final class Canonical {
  private const STRIP_KEYS = ['utm_source','utm_medium','utm_campaign','utm_term','utm_content','ref','fbclid','gclid','msclkid','_hsmi','_hsenc'];
  ...
  public static function withTrailingSlash(string $p): string { return self::isFileLike($p) ? $p : rtrim($p,'/').'/'; }
  public static function normalizePath(string $p): string {
    ...
    $norm = '/'.implode('/',$parts);
    return self::withTrailingSlash($norm);
  }
  public static function stripParams(array $q): array {
    $out=[]; foreach($q as $k=>$v){ if(!in_array(strtolower((string)$k), self::STRIP_KEYS, true)){ $out[$k]=$v; } } return $out;
  }
  public static function absolute(string $path): string {
    $host = $_SERVER['HTTP_HOST'] ?? 'nrlcmd.com';
    return 'https://'.$host.self::normalizePath($path);
  }
  public static function guardOneHop(): void {
    ...
    $qsNorm = http_build_query(self::stripParams($q));
    if ($qsNorm) $target .= '?'.$qsNorm;

    $current = $scheme.'://'.$host.self::withTrailingSlash($uri);
