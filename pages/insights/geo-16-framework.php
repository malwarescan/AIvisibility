...
require_once dirname(__DIR__, 2).'/lib/schema_utils.php';
...
// Insights articles: Article (+ optional VideoObject)
$PAGE = nc_page_url();
$orgId = nc_org_id();
$webpageId = nc_id('#webpage');

$headline = $ctx['title'] ?? 'AI Answer Engine Citation Behavior: The GEO-16 Framework Explained';
$imgUrl   = $cover_img ? nc_base_url().$cover_img : null;
$pubIso   = isset($published) ? date(DATE_W3C, strtotime($published)) : null;
$modIso   = isset($modified)   ? date(DATE_W3C, strtotime($modified))   : $pubIso;

$GLOBALS['articleSchema'] = [
  "@type" => "Article",
  "@id"   => nc_id('#article'),
  "headline" => $headline,
...
// If the article embeds a video, expose VideoObject
if (!empty($VIDEO)) {
  $GLOBALS['videoSchema'] = [
    "@type" => "VideoObject",
    "@id"   => nc_id('#video'),
    "name"  => $VIDEO['name'] ?? $headline,
    "description" => $VIDEO['description'] ?? $headline,
    "thumbnailUrl" => $VIDEO['thumbnailUrl'] ?? [],
    "uploadDate"   => $VIDEO['uploadDate'] ?? $pubIso,
